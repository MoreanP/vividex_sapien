...
class DexYCBVideoDataset:
    def __init__(self, data_dir="data", hand_type="right", filter_objects=[], only_grasped_objects=True):
        ...
        self.only_grasped_objects = only_grasped_objects
        ...
        for subject_dir in self._subject_dirs:
            for capture_dir in subject_dir.iterdir():
                ...
                self._subjects.append(subject_dir.stem)
    ...
    def __getitem__(self, item):
        ...
        subject_name = self._subjects[item]
        ...
        if self.only_grasped_objects:
            grasped_ycb_ids = ycb_ids[meta["ycb_grasp_ind"]]
            object_mesh_files = [self._object_mesh_file(grasped_ycb_ids)]
            ycb_ids = [grasped_ycb_ids]
            object_pose = object_pose[:, [meta["ycb_grasp_ind"]]]
        else:
            object_mesh_files = [self._object_mesh_file(ycb_id) for ycb_id in ycb_ids]
        
        object_names = [YCB_CLASSES[ycb_id] for ycb_id in ycb_ids]
