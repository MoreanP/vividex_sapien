...
class BaseSimulationEnv(object):
    def __init__(self, use_gui=False, frame_skip=10, no_rgb=False, need_offscreen_render=True, **renderer_kwargs):
        need_offscreen_render = need_offscreen_render
        engine, renderer = get_engine_and_renderer(use_gui=use_gui, need_offscreen_render=need_offscreen_render, no_rgb=no_rgb, **renderer_kwargs)
    ...
    def check_actor_pair_contacts(self, actors1: List[sapien.Actor], actor2: sapien.Actor, impulse_threshold=1e-2) -> np.ndarray:
        actor_set1 = set(actors1)
    ...
    def check_actors_pair_contacts(self, actors1: List[sapien.Actor], actors2: List[sapien.Actor], impulse_threshold=1e-2) -> np.ndarray:
        contact_buffer = np.zeros(len(actors1))
    ...
    def check_actor_pair_contacts_in_distances(self, actors1: List[sapien.Actor], actor2: sapien.Actor, centers: List[np.ndarray], radii: List[float], impulse_threshold=1e-2, reverse=False) -> np.ndarray:
        actor_set1 = set(actors1)
        contact_buffer = np.zeros(len(actors1))
        for contact in self.scene.get_contacts():
            contact_actors = {contact.actor0, contact.actor1}
    ...
    def check_actors_pair_contacts_in_distance(self, actors1: List[sapien.Actor], actors2: List[sapien.Actor], centers: List[np.ndarray], radii: List[float], impulse_threshold=1e-2, reverse=False) -> np.ndarray:
        contact_buffer = np.zeros(len(actors1))
        for actor2 in actors2:
            contact_buffer_local = self.check_actor_pair_contacts_in_distances(actors1, actor2, centers, radii, impulse_threshold, reverse=reverse)
            contact_buffer += contact_buffer_local
        contact_buffer = np.clip(contact_buffer, 0, 1)
        return contact_buffer
