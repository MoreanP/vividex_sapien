...
def get_engine_and_renderer(use_gui=True, use_ray_tracing=False, device="", mipmap_levels=1, need_offscreen_render=False, no_rgb=False, **kwargs):
    ...
    if use_ray_tracing:
        ...
        if need_renderer:
            _renderer = sapien.SapienRenderer(default_mipmap_levels=mipmap_levels, offscreen_only=not use_gui, device=device, do_not_load_texture=no_rgb)
...
def add_default_scene_light(scene: sapien.Scene, renderer: sapien.VulkanRenderer, add_ground=True, cast_shadow=True):
    ...
    scene.add_directional_light(np.array([-1, -1, -1]), np.array([1.0, 1.0, 1.0]), shadow=cast_shadow)
    scene.add_directional_light([0, 0, -1], [0.9, 0.8, 0.8], shadow=False)
    scene.add_directional_light([1, 1, 1], [2.0, 2.0, 2.0], shadow=False)
