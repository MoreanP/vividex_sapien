...
def generate_ur5_robot_hand_info() -> Dict[str, ArmRobotInfo]:
    xarm_path = Path("robot/ur5_description/")
    allegro_hand_ur5 = ArmRobotInfo(path=str(xarm_path / "ur5_allegro.urdf"), hand_dof=16, arm_dof=6, palm_name="right_gripper_palm_link", arm_init_qpos=[89/180*np.pi, -116/180*np.pi, 147/180*np.pi, -31/180*np.pi, 89/180*np.pi, 90/180*np.pi], root_offset=[0, 0, 0])
    info_dict = dict(allegro_hand_ur5=allegro_hand_ur5)
    return info_dict
...
def load_robot(scene: sapien.Scene, robot_name, disable_self_collision=True) -> sapien.Articulation:
    ...
    if disable_self_collision:
        ...
        if "ur5" in robot_name:
            if "allegro" in robot_name:
                for link_builder in robot_builder.get_link_builders():
                    if link_builder.get_name() in ["right_gripper_link_09", "right_gripper_link_05", "right_gripper_link_01", "right_gripper_link_13", "right_gripper_palm_link"]:
                        link_builder.set_collision_groups(1, 1, 17, 0)
        else:
            if "allegro" in robot_name:
                for link_builder in robot_builder.get_link_builders():
    ...
    if "free" in robot_name:
        ...
        arm_joint_names = ["right_shoulder_pan_joint", "right_shoulder_lift_joint", "right_elbow_joint", "right_wrist_1_joint", "right_wrist_2_joint", "right_wrist_3_joint"]
        for joint in robot.get_active_joints():
            name = joint.get_name()
            if name in arm_joint_names:
                joint.set_drive_property(*(1 * robot_arm_control_params), mode="force")
